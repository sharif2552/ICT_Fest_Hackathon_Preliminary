...
# Serializes the conflict-check + quota-check + insert critical section so
# concurrent create_booking calls can't both pass a check before either
# commits (prevents double-booking and quota-limit races).
_booking_lock = threading.Lock()

# Serializes the cancelled-check + refund + status-update critical section
# of cancel_booking so concurrent cancels of the same booking can't both
# pass the "not already cancelled" check (prevents duplicate refunds).
